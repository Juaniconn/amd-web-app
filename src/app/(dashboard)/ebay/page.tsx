  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <div className="flex overflow-x-auto gap-4 pb-2 sm:grid sm:grid-cols-2 lg:grid-cols-4 sm:pb-0">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="h-28 w-48 shrink-0 sm:w-auto animate-pulse rounded-xl bg-muted" />
          ))}
        </div>
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            className={`fixed top-4 right-4 z-50 px-4 py-3 rounded-xl shadow-xl text-sm font-medium flex items-center gap-2 toast-${toast.type}`}
          >
      ...
      <div className="flex overflow-x-auto gap-4 pb-2 sm:grid sm:grid-cols-2 lg:grid-cols-4 sm:pb-0">
        {[
          ...
          { label: "Valor Total", value: `$${(stats?.valorTotal || 0).toLocaleString()}`, icon: <DollarSign className="w-4 h-4" />, color: "text-warning" },
          ...
          <motion.div
            ...
            className="card-premium p-5 min-w-[200px] shrink-0 sm:min-w-0"
          >
      ...
      <div className="card-premium sticky top-0 z-10">
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground font-mono mt-0.5">{item.modelo}</p>
                </div>
              </div>

              <div className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-4">
                  <span className="text-muted-foreground">{item.fabricante}</span>
                  <span className="font-medium">Qty: {item.cantidad}</span>
                </div>
                <span className="text-base font-bold text-brand">${item.precio.toFixed(2)}</span>
              </div>
              ...
              <div className="flex items-center gap-2 pt-2 border-t border-border">
                ...
                <Link href={`/inventory/${globalIndex}`} className="p-2 rounded-md text-muted-foreground hover:bg-muted hover:text-foreground transition-colors">
                  <Eye className="w-4 h-4" />
                </Link>
                <Link href={`/inventory/${globalIndex}/edit`} className="p-2 rounded-md text-muted-foreground hover:bg-muted hover:text-foreground transition-colors">
                  <Edit className="w-4 h-4" />
                </Link>
              </div>
            </motion.div>
