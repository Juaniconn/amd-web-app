...
**Fecha:** 2026-08-14  
Este bloque documenta el sistema **tal como está compilado y migrado** tras Fase 4, más el cierre documental de Dirección (ADR-036–043). Las secciones 1–40 siguen siendo la especificación objetivo; no se borran. Numeración operativa vigente: **Fase 4 = Ingeniería y Diseño ✅** (ADR-032). No hay código de producción.
...
**Fase 4 Ingeniería:** `engineering_requests`, `engineering_hours`. Columnas RFQ/origen en `quotes` y `orders`. ADR-033 a ADR-042. Costeo completo, ECO, `documents.revision` y FK de `orders.engineering_request_id` **no** están en el DDL.

Deuda: snapshot Drizzle `0003` no materializado (SQL + journal sí). `orders.engineering_request_id` es texto sin FK.
...
CAD/CAM oficial (no integrado): SolidWorks, Mastercam, Fusion 360, AutoCAD (ADR-036). `liberado` = Aprobado para manufactura (ADR-038).

Campos de costeo oficiales **no** en DDL: horas estimadas, costo hora, costo total, tipo cobro `incluido|tarifa_fija|por_hora` (ADR-039). ECO: entidad `engineering_changes` ⬜ (ADR-040).
...
| `GET /api/documents/[id]` | ✅ sesión + `quotes:read` si `quote`; `engineering:read` si `engineering_request`; otros 403 |
...
- Archivos Ingeniería: máximo 50 MB; pdf/dwg/dxf/step/stp/iges/igs/png/jpg/jpeg/zip. Numeración solicitud `ING-YYYY-NNNNN`. Nomenclatura de archivo operativa `AMD-PART-XXXX_REV-*` (no validada en código).
