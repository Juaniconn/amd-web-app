...
`liberado` = sello de negocio **Aprobado para manufactura** (ADR-038). `revision_interna` incluye DFM (ADR-041).
...
| Revisión Interna | `revision_interna` | Revisión AMD + DFM (Ingeniería, CAM, Jefe de Taller) |
...
| Liberado | `liberado` | **Aprobado para manufactura.** Listo para cotización final y/o OP |
...
- OP de Fase 5 espera un plano **Liberado / Aprobado para manufactura** o un adjunto de RFQ directa ([[Origen Orden Produccion]], [[Control Documental]]).
- Tras `Liberado`, un cambio es [[ECO ECN]], no una transición de esta máquina.
